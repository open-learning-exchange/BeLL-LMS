    ...
    case 'resources':
      foreach($records as $record) {
        ...
        if($record->Community == "YES") {
			array_push($resAudience,"community education");
        }
        if($record->TLR == "YES") {
          array_push($resAudience,"teacher training");
        }
        if ($record->KG || $record->P1 || $record->P2 || $record->P3 || $record->P4 || $record->P5 || $record->P6) {
          array_push($resAudience,"formal education");
        }
		$n->audience = $resAudience;
    ...
    case 'teacherClass' :
      ...
      foreach($records as $record) {
        ...
        if(!$skip) {
          ...
          $n->kind = "Member";
          $n->facilityId = $facilityId;
          $personRole = array(strtolower($record->Role));
          if($record->Role=="Admin"){
              $personRole = array("teacher","leadteacher","headteacher","coach");
            }
          $n->roles = $personRole;
          $n->pass = $record->pswd;
          $n->status= "active";
